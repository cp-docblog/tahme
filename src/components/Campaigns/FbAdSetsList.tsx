...
import styles from './FbAdSetsList.module.css';
...
export const FbAdSetsList: React.FC<FbAdSetsListProps> = ({
    adAccountId,
    accessToken,
    campaignId,
    onSelectAdSet
    ...
    }, [campaignId, accessToken, dateRange.start, dateRange.end]);
    ...
    const fetchAdSets = async () => {
        ...
        try {
            const { data, error: fnError } = await supabase.functions.invoke('fb-adsets', {
                body: {
            ...
            console.error('Error fetching Facebook ad sets:', err);
    ...
    const fetchPerformanceStats = async () => {
        try {
            ...
            if (data && Array.isArray(data) && data[0]?.total_stats?.[0]?.total_stat?.breakdown_stats?.adset) {
                const adsets = data[0].total_stats[0].total_stat.breakdown_stats.adset;
                adsets.forEach((item: any) => {
    ...
    const handleAskAI = async () => {
        ...
        try {
            ...
            const adsetsData = adsets.map(adset => ({
                ...
                stats: performanceStats.get(adset.id) || {
                    spend: 0,
                    impressions: 0,
    ...
    const pausedAdSets = adsets.filter(a => a.status === 'PAUSED' || a.status !== 'ACTIVE');
    ...
    const sortAdSets = (list: AdSet[], metric: SortMetric, descending = true): AdSet[] => {
    ...
    const applyFilter = (list: AdSet[]): AdSet[] => {
    ...
    const topPerformers = useMemo(() => {
        return sortAdSets(adsets.filter(a => {
            const stats = performanceStats.get(a.id);
            return stats && stats.spend > 0;
        }), sortBy);
    }, [adsets, performanceStats, sortBy]);

    const topPerformer = topPerformers[0] || null;

    // Get ad sets to display based on active tab
    let displayAdSets: AdSet[] = [];
    if (activeTab === 'active') {
        displayAdSets = applyFilter(sortAdSets(activeAdSets, sortBy));
    } else if (activeTab === 'top') {
        displayAdSets = topPerformers;
    } else if (activeTab === 'paused') {
        displayAdSets = applyFilter(sortAdSets(pausedAdSets, sortBy));
    }
                ...
                {displayAdSets.length === 0 ? (
                    ...
                    <div className={styles.grid}>
                        {displayAdSets.map((adset) => {
                            const isTopPerformer = activeTab === 'top' && adset.id === topPerformer?.id;
                            const stats = performanceStats.get(adset.id);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={styles.adsquadHeader}>
                                        <h3 className={styles.adsquadName}>{adset.name}</h3>
                                        <span
                                            className={`${styles.statusBadge} ${adset.status === 'ACTIVE' ? styles.active :
                                                adset.status === 'PAUSED' ? styles.paused :
                                                    styles.inactive
                                                }`}
                                        >
                                            {adset.status === 'PAUSED' ? 'PAUSED' : adset.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className={styles.adsquadFooter}>
                                        <span className={styles.adsquadId}>ID: {adset.id.substring(0, 8)}...</span>
        ...
        <div className={styles.adsquadsList}>
            <div className={styles.stats}>
                <div className={styles.statCard}>
                    ...
                    <div>
                        ...
                        <div className={styles.statValue}>{adsets.length}</div>
                    </div>
                </div>
                <div className={styles.statCard}>
                    <DollarSign size={20} />
            ...
            <div className={styles.controls}>
                ...
                <Button
                    onClick={handleAskAI}
                    variant="secondary"
                    disabled={adsets.length === 0 || loading}
                >
                    <Sparkles size={18} />
                    {t('reports.askAI')}
                </Button>
