...
serve(async (req) => {
    ...
    try {
        ...
        if (profile.role !== 'owner' || !profile.is_active) {
            return new Response(
                JSON.stringify({ error: 'Unauthorized: Only owners can change user roles' }),
        ...
        if (!user_id || !new_role) {
            return new Response(
                JSON.stringify({ error: 'Missing required fields: user_id, new_role' }),
                { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }

        // Validate role
        if (!['owner', 'admin', 'staff'].includes(new_role)) {
            return new Response(
